...
App.IndexController = Ember.ArrayController.extend({
  // worksCount: function() {
    ...
    // return this.count1;
  // }.property('length'),
  worksCount: Ember.computed.alias('length'),
  artistsCount: 3,
  logo: 'images/ga-logo.png',
  workViews: function() {
    return this.filterBy('workViews', true).slice(0,3);
    // return this.filter(function(work) {
    //   return work.get('workViews');
    // });
  }.property(),
  workViewsCount: function() {
    
    // var sorting = this.sortBy('workViewsCount').slice(0,3);
    
    // return sorting
    // sortAscending: false;
    var workTotal = this.get('length');
    return this.sortBy('workViewsCount').slice(workTotal-3,workTotal);
    
    // return this.filter(function(work) {
    //   return work.get('workViewsCount');
    // });
  }.property(),
  artistViews: function() {
    
    // return this.filterBy('artistViews', true).slice(0,3);
    return this.filter(function(artist) {
      return artist.get('artistViews');
    });
  }.property()
});
...
App.Work = DS.Model.extend({
  ...
  workViews: DS.attr('boolean'),
  workViewsCount: DS.attr('number'),
