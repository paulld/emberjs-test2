...
App.Router.map ->
  ...
  @resource 'works', ->
    @route 'onsale'
  @resource 'work', { path: '/works/:work_id' }
  @resource 'artists', ->
...
App.IndexRoute = Ember.Route.extend(
  model: ->
    @store.findAll('work')
    # works: @store.findAll('work')
    # artists: @store.findAll('artist')
)
...
#   model: ->
#     // console.log(params)
#     @store.find('work', params.work_id)
...
App.IndexController = Ember.ArrayController.extend(
  ...
  workViewCount: ( ->
    # @sortBy('workViewCount').slice(0,3)
    # sorting
    # sortAscending: false
  #  workTotal = 5
    workTotal = @get('length')
    @sortBy('workViewCount').slice(workTotal-3,workTotal)
    # @filter = (work) ->
    #   work.get('workViewCount')
    # @filter = (work) ->
    #   sortAscending: false
    #   @sortBy('workViewCount').slice(0,3)
